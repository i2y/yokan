...
CATALOGUE = [
  ["start", "counter",
   ...
   "基本形。アプリはオブジェクトで、状態はそのインスタンス変数、ハンドラはそれを閉じ込めたブロック"],
  ["start", "blockform",
  ...
  ["start", "calc",
   ...
   "電卓。累算器ひとつと保留中の演算ひとつ、見た目は Hash にまとめて各キーに `**` で渡す"],
  ["start", "calcgrid",
  ...
  ["state", "links",
   ...
   "互いを指すオブジェクト。Ruby の回収器は循環を扱えるので、逆向きの参照も普通の参照でよい"],
  ["state", "moods",
  ...
  ["look", "layout",
   ...
   "spacer と divider。続きを端まで押しやる詰め物と、区切り線"],
  ["look", "cards",
  ...
  ["look", "styled",
   ...
   "見た目をひとところに。Hash を merge して `**` で渡し、`theme:` で一枚まるごと切り替える"],
  ["look", "badges",
  ...
  ["ruby", "edges",
   ...
   "端の話。リストの終わりを越えた添字と、機械語の 1 語をはるかに越えた数"],
  ["ruby", "flow",
