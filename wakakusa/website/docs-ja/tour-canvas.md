...
`scale` は、その一つが画面上でいくつぶんになるかです。
64 かける 40 の格子を 6 で描けば、画面上では 384 かける 240 になります。
...
名前は素直なもので、`"left"`、`"right"`、`"up"`、`"down"`、`"space"`、`"enter"` があり、文字のキーはその文字そのものです。
...
どちらも、原作と違って無音のままです。
エンジンにまだ音を鳴らす仕組みがないからです。
原作が生成器に種を与えているところは、それぞれ算術だけの六行に置き換えてあります。
種を与えた `Random` は二つの実行で同じ生成器にならないので、コマを比べられないゲームでは、ゲートに通す意味がなくなります。
