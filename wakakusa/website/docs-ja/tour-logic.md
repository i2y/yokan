...
片方の枝で何も書かない `if` は、ただ何も書きません。
場所取りの要素も、`nil` に備える書き方も要りません。
...
繰り返しは行に必要なものを渡してそれを呼び、ボタンのブロックは繰り返しの変数ではなくメソッドの引数を閉じ込めます。
...
だからアプリが持つのは一覧と番号で、選ばれた文字列の写しではありません。
...
二つのグラフは、数の一覧を第一引数に取ります。
だから数を計算してから描くビューが、上から順に読めます。
