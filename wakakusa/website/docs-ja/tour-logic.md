...
ブロックから呼んだメソッドが作った要素も、開いているコンテナに加わります。
...
数を持つ二つの欄は、改行するか欄から離れると確定します。
数でない文字は捨て、値は `min` と `max` の範囲に収め、`step` に合わせます。
...
ハンドラの渡し方はその二つだけです。
メソッド名のシンボルを渡す書き方は断ります。
そのときは、代わりにどう書くかもエラーに出ます（[若草が断る書き方](refusals.md)）。
...
行数が少なく、すでに手元にある場合は、`data_table` に行をそのまま子として渡します。
最初の `row` が見出しで、以降がデータ行になり、交互に色が変わります。
表の枠は `data_table` が描きます。
...
- [キャンバスとキーボード](tour-canvas.md)：絵を描く面と、キーボードの読み方。
