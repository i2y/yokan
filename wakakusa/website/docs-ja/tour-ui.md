...
画面をどう並べて何色にするかと、要素ではなくウィンドウが渡すものです。

## 並べる

`column` と `row` は、子を上下と左右に並べます。
並べかたの違う箱がもう四つあり、どれも `column` と同じように、子を引数としてもブロックとしても取ります。
...
`modal` は `open:` のあいだ、ウィンドウの残りの上に出る一枚です。
ダイアログはこれで書きます。
アプリがあると言えばあり、ないと言えばありません。
...
`demo/panels.rb` が四つの箱を一画面に、`demo/dialog.rb` が modal を見せます。
...
| `theme` | `"dark"` か `"light"` | ここから下が色を解決する配色 |
| `animate` | 数 | 変化にかける時間、ミリ秒 |
| `easing` | `"linear"` `"in"` `"out"` `"inOut"` | その変化の曲がり方 |
...
`text` の `width` は文字の幅を指し、`image` の `width` と `height` は画像そのものの大きさなので、外側の箱は手を出しません。
どの要素がそうなのかは[要素](elements.md)に印をつけてあります。

`demo/shared.rb` は、共通の性質を一つずつ違う種類の要素に付けたものです。
spacer にテーマ、選択にアニメーション、区切り線に説明、フィールドに二列ぶんの幅、そして欄とボタンを触れなくする錠です。
...
リテラルではなく値を取るので、アプリの側で切り替えられます（`theme: @mode`）。
...
ダイアログは人の操作を待つので、`task` の中に置きます（[Ruby、データ、仕事](tour-lib.md#ウィンドウの外の仕事)）。
