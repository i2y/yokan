...
若草のコマンドは、その読み取りに合う形にしてあります。
三つのうち二つはコンパイラを起動せず、ウィンドウも開きません。
それぞれが答える問いは一つずつです。
この書き方は語彙の内側か、これは何を描くか、リリースしたアプリも同じことをするか。

![エージェントが回る環。中央の app.rb を書き、wakakusa check とウィンドウなしの dump を 1 秒かからずに回り、仕上がったら環を出て wakakusa gate へ、そしてリリースへ進む](images/loop-ja.svg#only-dark)

![エージェントが回る環。中央の app.rb を書き、wakakusa check とウィンドウなしの dump を 1 秒かからずに回り、仕上がったら環を出て wakakusa gate へ、そしてリリースへ進む](images/loop-ja-light.svg#only-light)

## 三つのコマンド、三つの答え

### `wakakusa check`：この書き方は語彙の内側か
...
語彙の内側にあるアプリには、何も言いません。
コンパイラを起動しないので、答えは 0.1 秒ほどで返ります。

文面は「だめだ」ではありません。
直しかたが、直す場所に書いてあります。
一覧は[若草が断る書き方](refusals.md)にあり、それぞれ引き金になるコードも載せてあります。
...
手順の語彙は[検証と配布](tour-ship.md#人のいない実行)にあります。
...
最後の一つが約束で、仕事が終わったと言う前に走らせるものです。
...
これを読んだエージェントは、断りを一つずつ受け取りながら覚えるのではなく、最初から語彙どおりに書きます。
