...
#### blockform — 同じカウンタを別の書き方で。コンテナは子をブロックとして取り、どちらの書き方も同じ木になる
...
#### control — ビューの中はただの Ruby。`if`、`unless`、三項演算子、ループ、画面の一部を返すメソッド
...
#### moods — いくつかの名前のどれかである値（シンボル）と、何もないかもしれない値（nil）を `case` で見分ける
...
#### cards — 名前のついた画面の一部はメソッド。他の要素を包むものは、それらを引数に取る
...
#### canvas — 仮想ピクセルの格子を、命令をひとつずつ並べて塗る。色はパレットの番号、キーボードはタイマーから読む
...
#### stdlib — Ruby 自身の標準ライブラリをゲートにかける。`Math`、`Time`、`JSON`、`CSV`、`format`、正規表現、`Enumerable`
...
#### files — Ruby 自身の `File` と `Dir` でファイルを扱う。ここに Wakakusa のものは何もない
...
#### flow — ハンドラの中の制御フロー。飛ばすループ、止まるループ、while、別のメソッドを包むメソッド
