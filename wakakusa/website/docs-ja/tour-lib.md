...
実装が二つあるライブラリについて確かめられるのは、そこまでです（[二つの実行](two-runs.md)）。
...
ここは二つの実行で違いが出ますが、速いのはリリースしたほうです。
コンパイルされた実行では、二本のスレッドが二つのコアで走ります。
どちらのスレッドも二秒かかる計算なら、全体でも二秒で終わり、ウィンドウはいつもの速さで描き続けます。
同じアプリを CRuby で動かすと倍かかります。
インタプリタはスレッドを一度に一本しか走らせないので、処理が終わるまでウィンドウが止まります。
