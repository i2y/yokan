...
var catalogue = []entry{
	{"start", "counter",
		...
		"基本形。アプリは構造体で、状態はそのフィールド、ハンドラはフィールドを閉じ込めたクロージャ"},
	{"start", "prefixed",
		...
		"同じカウンタを、パッケージに名前を付けて import した形で。呼び出しはすべて `gm.` から始まり、アプリは自分の型にどんな名前でも使える"},
	{"start", "control",
		...
		"ビューの中はただの Go。`if`、ループ、画面の一部を返すメソッド。要素のスライスを、ほかの Go と同じ書き方で組み立てる"},
	{"start", "todo",
	...
	{"start", "calc",
		...
		"電卓。途中の値ひとつと待っている演算ひとつを持ち、見た目は各キーに同じ属性を付ける関数にまとめる"},
	{"start", "calcgrid",
	...
	{"state", "lookup",
		...
		"アプリが持つ map。既定値つきの読み出し、鍵があるかどうかの確認、ウィンドウを開けたままの追加。ビューの中で map を走査することはない"},
	{"state", "points",
	...
	{"look", "styled",
		...
		"見た目をひとところに。同じ属性をどのボタンにも付ける関数と、パネルを丸ごと切り替える `Theme`"},
	{"look", "badges",
	...
	{"go", "edges",
		...
		"端の話。終わりを越えた添字はどちらの実行でもプログラムを止めるので、アプリは先に長さを確かめる。マシンの語長を超えた数は `math/big` で、これも両方が同じパッケージ"},
	{"go", "flow",
	...
	{"work", "tasks",
		...
		"時間のかかる処理を、ウィンドウのスレッドの外の goroutine で。答えは二つ目のクロージャで受け取る"},
}
