...
44 apps, every one of them gated: the interpreted run and the compiled
...
#### counter — the reference: an app is a struct, its state is its fields, and a handler is a closure over them
...
#### prefixed — the counter again with the package imported under a name: every call reads `gm.`, and the app may use any name for its own types
...
#### control — ordinary Go inside a view: `if`, a loop, and a method that answers part of the screen; the view builds a slice of elements the way any Go does
...
#### calc — a calculator: one accumulator, one pending operation, and a look that is a function setting the same properties on every key
...
#### calcgrid — the same calculator on a grid instead of five rows; `ColSpan` is what makes the zero key twice as wide
...
#### lookup — a map on the app: read with a fallback, asked whether a key is there, added to while the window is open, and never ranged over in a view
...
#### points — a small struct of values, carried on the app's own state
...
#### moods — values that are one of a few named things (constants) and a value that may be nothing (a pointer that may be nil)
...
#### links — objects that point at one another; Go's collector takes a cycle in its stride, so the back pointer is an ordinary pointer
...
#### styled — a look kept in one place: a function that sets the same properties on every button, and `Theme` flipping a whole panel
...
#### shared — the methods every element has, on elements that have nothing else in common
...
#### table — `DataTable` draws the table itself: the first row is the header, the later ones are shaded in alternation
...
#### stdlib — Go's own under the gate: `math`, `sort`, `time`, `encoding/json`, `encoding/csv`, `strings`, `regexp` — the same compiled packages in both runs
...
#### files — files with Go's own `os`; nothing here is Gomamochi's, and the gate says both runs answer the same
...
#### reader — a page fetched off the window's thread from a server the app runs itself, so both runs read the same bytes
...
#### dbnotes — a database reached through the engine, one implementation for both runs, with the values bound rather than spliced
...
#### ledger — money kept in a database: an item called o'brien is an apostrophe and never a piece of SQL
...
#### edges — an index past the end stops the program in both runs, so the app asks the length first; a number past a machine word is `math/big`, the same package in both
...
#### flow — control flow in the handlers: a loop that skips, a loop that stops, a `for` with a condition, and a method that wraps another
...
#### tasks — work that takes a while, done off the window's thread on a goroutine of its own; the answer comes back through the second closure
