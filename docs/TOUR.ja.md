...
`{ |x: Int| … }` はクロージャです。
ローカル変数に入れ、メソッドに渡し、状態のフィールドに持たせ、動いている最中に別のものへ差し替えられます。

```ruby
store Board {
  state n : Int = 0
  state step : fn(Int) -> Int = { |x: Int| x + 1 }   # フィールドが持つ

  fn twice {
    let double = { |x: Int| x * 2 }                  # ローカルが持つ
    n = double(n)
  }

  fn apply(g: fn(Int) -> Int, v: Int) Int {          # 引数として受け取る
    g(v)
  }

  fn through {
    n = Board.apply({ |x| x * 10 }, n)               # 型は渡し先の署名から
  }

  fn advance {
    let f = step                                     # フィールドを読んでから呼ぶ
    n = f(n)
  }
}
```

クロージャはメソッドの本体と同じように World を受け取るので、読むだけでなく書くこともできます。
そのためビューからは呼べませんし、ハンドラの中ではなく `fn` の中で作ります。
引数の型は、書かれたもの（`|x: Int|`）か、入れる先（注釈付きの `let`、宣言された引数、フィールド）から決まります。
本体から推論することはなく、できないときはそう言って断ります。

捕獲は値で、クロージャを作った時点のものです。
ハンドラと同じ捕獲規則で、Copy なハンドルと値だけを捕まえられます。
捕まえたハンドルは古くなることがありますが、その読み出しは解放済みの対象に触れるのではなく、古いハンドルがいつも答えるものを答えます。
...
- ハンドラの中で書くクロージャと、ビューからのクロージャ呼び出し。
  関数値は World を受け取って書き換えもできるので、作るのも呼ぶのも `fn` の中です。
  引数の型は、書かれたものか入れる先から決まり、本体からは決まりません
- HTTP **サーバー**（構想では宣言的な `service` ブロックです。意図して後回しにしています）
- Windows（エンジンの動作を確かめているのは macOS と Linux です）
