...
同じ操作の台本を両方に流し、出てきた画面を突き合わせるコマンドです（[ヘッドレス実行とゲート](#ヘッドレス実行とゲート)）。
...
6. [関数を値として扱う](#関数を値として扱う)
7. [算術](#算術)
8. [文字列](#文字列)
9. [リスト、チャート、仮想化リスト](#リストチャート仮想化リスト)
10. [キャンバス](#キャンバス)
11. [辞書](#辞書)
12. [タプル](#タプル)
13. [Value クラスとインターフェース](#value-クラスとインターフェース)
14. [メモリ管理](#メモリ管理)
15. [直和型と match](#直和型と-match)
16. [Optional と Enum](#optional-と-enum)
17. [コンポーネント](#コンポーネント)
18. [共通のプロパティ](#共通のプロパティ)
19. [スタイルとテーマ](#スタイルとテーマ)
20. [アニメーション](#アニメーション)
21. [ウィンドウ](#ウィンドウ)
22. [エラー処理](#エラー処理)
23. [標準ライブラリ](#標準ライブラリ)
24. [Rust crate を呼ぶ](#rust-crate-を呼ぶ)
25. [パッケージ](#パッケージ)
26. [CPython エスケープ](#cpython-エスケープ)
27. [重い処理とタイマーとキー](#重い処理とタイマーとキー)
28. [型チェッカーとの併用](#型チェッカーとの併用)
29. [エディタ](#エディタ)
30. [テスト](#テスト)
31. [ヘッドレス実行とゲート](#ヘッドレス実行とゲート)
32. [リリース](#リリース)
33. [本格アプリの例](#本格アプリの例)
34. [今できないこと](#今できないこと)
...
チャートにポインタを載せると、その下の値が読めます。棒や点のラベル（ラベルがなければ `#3` のような番号）と、系列ごとの数値です。
スクリプトでは `hover:<i>`（i 番目の点。n 番目のチャートなら `hover@n:<i>`、棒グラフと折れ線グラフを通して数えます）でポインタを載せ、`hover:` で外します。ダンプにその読み取りが載るので、ホバーで見えるものもクリックと同じように確かめられます。
...
描画命令は要素ではありません。
[共通のプロパティ](#共通のプロパティ)は一つも取れません。
キャンバスの中のものはクリックできず、アクセシビリティのツリーではキャンバス全体が1枚の画像です。
...
- **`role=` / `a11y_label=`**：`role=` は、要素が自分で決めている役割（スクリーンリーダーの "button"、"heading"、"list" など）を上書きします。
  ...
  ヘッドレススクリプトの `a11y` ステップが、そのツリーを出力します（`demo/labels.py`）。
...
どのモジュールが Python のどこまでを実装しているかは、関数の単位で[対応状況のページ](https://i2y.github.io/yokan/ja/support/)にあります（手で書かずに生成しています）。
...
- **keys**：`down(k)` / `pressed(k)` / `released(k)` — 装置としてのキーボード。
  タイマーのティックから読む（ハンドラに届くキーの組み合わせのほうは[ウィンドウ](#ウィンドウ)を参照）
...
アプリは複数のファイルで書けます。
エントリの隣のモジュールは名前で import でき（`from widgets import badge`）、同じプログラムにコンパイルされます。
パッケージはその一歩先です。
普通にインストールされた Python のパッケージが、それを import したアプリの中にコンパイルされます。

方言で書かれていることは、`__init__.py` の隣に置く `py.yokan` というファイルで示します。
型付きパッケージが `py.typed` で示すのと同じ形です。

```
yokanui/
    __init__.py
    py.yokan            # 空のファイル。「これは方言のコード」
    badges.py
    panels.py
```

```toml
# pyproject.toml。目印はパッケージと一緒に配られる必要があります
[tool.setuptools.package-data]
yokanui = ["py.yokan"]
```

アプリ側の書き方は、ほかの import と変わりません。
...
パッケージの中では相対 import が使えます（`from .badges import badge`）。
`__init__.py` で、そのパッケージが提供するものを再公開します。
実行時にライブラリは存在せず、アプリの隣に配るものもありません。
パッケージのモジュールはアプリ自身のファイルと同じように読まれ、バイナリがそれを抱えます。

**名前はぶつかりません。**
パッケージ由来の名前は、そのモジュールから作った名前で出力されます。
パッケージの `badge` とアプリの `badge_of` は共存し、どちらの書き手も相手を知らずに済みます。
Python からは見えません。
アプリは `badge(...)` と呼びますし、`.pix` の中でだけ `YokanuiBadgesBadge` として見えます。

**パッケージは Rust を持てます。**
自分の `[tool.yokan.crates]` は、目印の隣の `yokan.toml` に置きます。
wheel は `pyproject.toml` を運ばないからです。
...
これはアプリの宣言に併合されます。
アプリ側はその crate に一度も触れません。
同じ crate を違うバージョンで宣言した二者は、理由を挙げて断ります。
一つのプログラムの中で、一つの crate は一つのバージョンだからです。

目印の無いインストール済みパッケージは、その名前が使われる場所で断り、メッセージが目印の名を挙げます。
方言のコードでないパッケージは、`@py` が動かすものです。

パッケージ自身は `@py` を持てません。
エスケープはビルドの中に Python を持ち込むもので、その Python が必要とするものを宣言するのは、依存をインストールするアプリの側だからです。

デモは `demo/pkg/`（パッケージ）と `demo/pkgapp.py`（それを import するアプリ）です。
...
ウィンドウは開かず画面を出力し、`--frames` を付けると各ステップのキャンバスを PNG で残します。
...
`check` と `show` とゲートがどう一つの往復になり、なぜエージェントが自分で回せるのかは、[エージェントと作る](https://i2y.github.io/yokan/ja/agents/)にまとめました。
...
コンパイル先の Rust クレート群はリポジトリに入っていますが、最初のネイティブビルドが、使っているバージョンに合うチェックアウトを `~/.cache/yokan/` に取ってきます。
...
同梱するライブラリは一つの表が決めていて、開発中に使う wheel も同じ表を読みます。
`libfontconfig` や `libxkbcommon` を同梱すると、ホストが持っているフォント設定やキーボードのデータとぶつかります。
それが Linux のパッケージの壊れ方なので、その手のものは外して、動かす機械のものを使います。
`--carry-libs` はこれを逆にするフラグです。
配る先にライブラリがないかもしれないときのために、C ランタイム以外をすべて運びます。
...
- **モデルのリストをビューで直接繰り返すこと**。
  ビューの `for` が回せるのは、スカラーと value クラスのリストです。
  モデルなら、表示したい文字列をストア側で組み立ててから `list_view` に渡します。
...
- **新しい要素の周辺**。
  ...
  ツールチップの表示は、スクリプトからホバーで確かめられません。
  コンテキストメニューのパネルも、スクリプトからは開けません。
  どちらも中身はダンプに出ます（ツールチップの文字列と、メニューの項目。項目は `select:` で選べます）。
  キーボードに関わるものは、ヘッドレスの検証にまだ無いステップを待っています。
  ドラッグの方はもう動かせます（`split` の仕切りは `slide:` で動きます）。
  だから列幅の変更が待っているのは、検証の仕組みではなく要素の側です。
- **二つめのウィンドウ**。
  いまは一アプリに一ウィンドウです。
...
- **Windows**。
  プラットフォーム自身の答えはツリーに入っています。
  キャッシュとアプリ自身のディレクトリの置き場所、ビルドが作る `.exe`、`--app` がそこに書くフォルダ（`--bundle`、`--onefile`、`--appimage` は名前を挙げて止まります）、そして Ctrl を指す `cmd` です。
  Windows のランナーでビルドとテストとゲートを回す CI のジョブは、緑になりました。
  ワークスペースのテスト、tier gate、両方の実行を突き合わせるデモ、そして作ったのとは別の機械で動かすパッケージ済みのフォルダまで通っています。
  足りないのはウィンドウです。
  そのランナーでは gpui が `Error creating DirectWriteTextSystem` で止まるので、アプリは画面をテキストで出せても描けません。
  実機の Windows で誰かがウィンドウを開くまで、入れられるのは macOS と Linux です。
  いま入れられるのは macOS と Linux です。
