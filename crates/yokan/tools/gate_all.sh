...
# Two panes and a rule that drags: `ratio` is the app's number, so
# `slide:` is what proves it, and the last two steps land outside the
# demo's own clamp — the app holds the floor, not the widget.
