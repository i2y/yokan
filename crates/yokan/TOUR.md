...
- **Around the new elements**: a table's columns cannot be resized by dragging, and its rows have no keyboard navigation or multi-select; charts have no legend; `select` has no keyboard operation; a tooltip's appearance is not something a script can hover for, and a context menu's panel is not something a script can open (both are in the dump: the tooltip's text, and the menu's items, which `select:` picks from). The keyboard ones wait on a verb the headless harness does not have. A drag has one now — `split`'s divider is driven by `slide:` — so a resizable column waits on the widget rather than on the harness.
...
- **Windows.** The platform's own answers are in the tree: where a cache and an app's own directory go, the `.exe` a build produces, the folder `--app` writes there (`--bundle`, `--onefile` and `--appimage` name themselves and stop), and `cmd` meaning Ctrl. The CI job that builds, tests and gates on a Windows runner has now run green — the workspace's tests, the tier gate, demos through both runs, and the packaged folder driven headless on a machine that never built it. What is missing is the window: gpui stops at `Error creating DirectWriteTextSystem` on that runner, so an app there prints its screen and does not draw one. Until someone opens a window on real Windows hardware, the platforms you can install are macOS and Linux.
