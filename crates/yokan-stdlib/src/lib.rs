...
// semantics (empty `min` raises, `sorted` is stable), and the gate
// holds the two runs together. The operations that say nothing about
// the element — `in`, a slice, `+`, `[::-1]` — are not here: they are
// generic on the kernel's own list, so they work for a value class or
// a tuple as well as for an int.
