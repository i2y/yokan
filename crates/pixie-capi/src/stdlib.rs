...
fn call(id: i32, args: &[Arg]) -> i64 {
    match id {
        ...
        12 => yokan_stdlib::fs_size(s(args, 0)),  // fs.size
        13 => yokan_stdlib::fs_modified_ms(s(args, 0)),  // fs.modified_ms
        14 => (yokan_stdlib::fs_is_dir(s(args, 0))) as i64,  // fs.is_dir
        15 => text_answer(yokan_stdlib::fs_read_text_from(s(args, 0), i(args, 1))),  // fs.read_text_from
        16 => yokan_stdlib::sqlite_exec(s(args, 0), s(args, 1)),  // sqlite.exec
        17 => list_answer(yokan_stdlib::sqlite_query_text(s(args, 0), s(args, 1))),  // sqlite.query_text
        18 => yokan_stdlib::sqlite_query_int(s(args, 0), s(args, 1)),  // sqlite.query_int
        19 => yokan_stdlib::sqlite_query_int_or(s(args, 0), s(args, 1), i(args, 2)),  // sqlite.query_int_or
        20 => list_answer(yokan_stdlib::sqlite_query_text_or(s(args, 0), s(args, 1))),  // sqlite.query_text_or
        21 => yokan_stdlib::sqlite_exec_with(s(args, 0), s(args, 1), l(args, 2)),  // sqlite.exec
        22 => list_answer(yokan_stdlib::sqlite_query_text_with(s(args, 0), s(args, 1), l(args, 2))),  // sqlite.query_text
        23 => yokan_stdlib::sqlite_query_int_with(s(args, 0), s(args, 1), l(args, 2)),  // sqlite.query_int
        24 => yokan_stdlib::sqlite_query_int_or_with(s(args, 0), s(args, 1), i(args, 2), l(args, 3)),  // sqlite.query_int_or
        25 => list_answer(yokan_stdlib::sqlite_query_text_or_with(s(args, 0), s(args, 1), l(args, 2))),  // sqlite.query_text_or
        26 => rows_answer(yokan_stdlib::sqlite_query_rows_all(s(args, 0), s(args, 1))),  // sqlite.query_rows
        27 => rows_answer(yokan_stdlib::sqlite_query_rows(s(args, 0), s(args, 1), l(args, 2))),  // sqlite.query_rows
        28 => rows_answer(yokan_stdlib::sqlite_query_rows_or_all(s(args, 0), s(args, 1))),  // sqlite.query_rows_or
        29 => rows_answer(yokan_stdlib::sqlite_query_rows_or(s(args, 0), s(args, 1), l(args, 2))),  // sqlite.query_rows_or
        30 => yokan_stdlib::clipboard_set_text(s(args, 0)),  // clipboard.set_text
        31 => text_answer(yokan_stdlib::clipboard_get_text()),  // clipboard.get_text
        32 => (yokan_stdlib::keys_down(s(args, 0))) as i64,  // keys.down
        33 => (yokan_stdlib::keys_pressed(s(args, 0))) as i64,  // keys.pressed
        34 => (yokan_stdlib::keys_released(s(args, 0))) as i64,  // keys.released
        35 => yokan_stdlib::audio_play(s(args, 0)),  // audio.play
        36 => yokan_stdlib::audio_play_at(s(args, 0), n(args, 1)),  // audio.play
        37 => yokan_stdlib::audio_stop(),  // audio.stop
        38 => { yokan_stdlib::notify_send(s(args, 0), s(args, 1)); 0 },  // notify.send
        39 => text_answer(yokan_stdlib::http_get_text(s(args, 0))),  // http.get_text
        40 => text_answer(yokan_stdlib::http_get_text_or(s(args, 0), s(args, 1))),  // http.get_text_or
        41 => text_answer(yokan_stdlib::http_get_text_timeout(s(args, 0), i(args, 1))),  // http.get_text
        42 => text_answer(yokan_stdlib::http_post_text(s(args, 0), s(args, 1))),  // http.post_text
        43 => text_answer(yokan_stdlib::http_post_text_as(s(args, 0), s(args, 1), s(args, 2))),  // http.post_text
        44 => text_answer(yokan_stdlib::http_post_text_or(s(args, 0), s(args, 1), s(args, 2))),  // http.post_text_or
        45 => yokan_stdlib::http_status(s(args, 0)),  // http.status
        46 => text_answer(yokan_stdlib::json_get_text(s(args, 0), s(args, 1))),  // jsondoc.get_text
        47 => yokan_stdlib::json_get_int(s(args, 0), s(args, 1)),  // jsondoc.get_int
        48 => num_answer(yokan_stdlib::json_get_float(s(args, 0), s(args, 1))),  // jsondoc.get_float
        49 => (yokan_stdlib::json_get_bool(s(args, 0), s(args, 1))) as i64,  // jsondoc.get_bool
        50 => yokan_stdlib::json_length(s(args, 0), s(args, 1)),  // jsondoc.length
        51 => (yokan_stdlib::json_has(s(args, 0), s(args, 1))) as i64,  // jsondoc.has
        52 => list_answer(yokan_stdlib::json_get_texts(s(args, 0), l(args, 1), s(args, 2))),  // jsondoc.get_texts
        53 => yokan_stdlib::strings_to_int(s(args, 0), i(args, 1)),  // strings.to_int
        54 => num_answer(yokan_stdlib::strings_to_float(s(args, 0), n(args, 1))),  // strings.to_float
        55 => text_answer(yokan_stdlib::clock_format_ms(i(args, 0), s(args, 1))),  // clock.format_ms
        56 => text_answer(yokan_stdlib::clock_format_local_ms(i(args, 0), s(args, 1))),  // clock.format_local_ms
        57 => yokan_stdlib::clock_local_offset_minutes(i(args, 0)),  // clock.local_offset_minutes
