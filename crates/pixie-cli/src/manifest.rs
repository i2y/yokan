...
///
/// Every manifest this crate writes is written as TEXT, to keep the
/// layout a person wrote; a value dropped into it raw is a value the
/// reader may not get back. A Windows path is the case that says so
/// out loud — `C:\Users\…` is four escapes and a parse error — and a
/// quote in a path does the same on any platform.
    ...
    pub fn dep_lines(&self) -> String {
        ...
        for c in &self.crates {
            ...
            if let Some(v) = &c.version {
                parts.push(format!("version = {}", toml_str(v)));
            }
            if let Some(p) = &c.path {
                parts.push(format!("path = {}", toml_str(&p.display().to_string())));
            }
            if !c.features.is_empty() {
                let fs: Vec<String> = c.features.iter().map(|f| toml_str(f)).collect();
...
fn derive_rpi(m: &Manifest, c: &CrateDep) -> Result<String, String> {
    ...
    if let Some(v) = &c.version {
        spec.push(format!("version = {}", toml_str(v)));
    }
    if let Some(p) = &c.path {
        spec.push(format!("path = {}", toml_str(&p.display().to_string())));
    }
    if !c.features.is_empty() {
        let fs: Vec<String> = c.features.iter().map(|f| toml_str(f)).collect();
...
mod toml_str_tests {
    ...
    /// The case Windows found: a path is four backslashes and a parse
    /// error when it is written raw. Read back with the same parser the
    /// manifest reader uses, the value has to come out as it went in.
    #[test]
    fn a_windows_path_survives_the_round_trip() {
        let p = r"C:\Users\RUNNER~1\AppData\Local\Temp\pixie";
        let text = format!("[dependencies]\nkit = {{ path = {} }}\n", toml_str(p));
        let doc: toml::Value = toml::from_str(&text).expect("valid TOML");
        assert_eq!(doc["dependencies"]["kit"]["path"].as_str(), Some(p));
    }

    /// And the ones every platform can produce.
    #[test]
    fn quotes_newlines_and_control_characters_survive_too() {
        for v in ["a\"b", "a\\b", "a\nb", "a\tb", "a\u{7}b"] {
            let text = format!("k = {}\n", toml_str(v));
            let doc: toml::Value = toml::from_str(&text).expect("valid TOML");
            assert_eq!(doc["k"].as_str(), Some(v), "{v:?}");
        }
    }
}
