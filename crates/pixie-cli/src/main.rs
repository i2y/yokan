...
fn write_crate(
    ...
    let engine_dep = match engine {
        Some(e) => format!(
            "pixie-engine-gpui = {{ path = {} }}\n",
            manifest::toml_str(&e.display().to_string())
        ),
        None => String::new(),
    };
    let interp_dep = match interp {
        Some(i) => format!(
            "pixie-interp = {{ path = {} }}\n",
            manifest::toml_str(&i.display().to_string())
        ),
        None => String::new(),
    };
    let patch = match (engine, vendored_gpui_macos_dir()) {
        (Some(_), Some(v)) => format!(
            "\n# Vendored lower half: pixie's patched gpui_macos (P1).\n\
             ...
             gpui_macos = {{ path = {} }}\n",
            manifest::toml_str(&v.display().to_string())
        ),
        _ => String::new(),
    };
    let manifest = format!(
        "# Generated by pixie — do not edit.\n\
         ...
         strip = \"debuginfo\"\n\n\
         [workspace]\n{patch}",
        manifest::toml_str(name),
        manifest::toml_str(&kernel.display().to_string())
    );
