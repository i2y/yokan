...
この二つが同じに振る舞うかどうかは、`yokan gate` で確かめられます。
クリックやキー入力の台本を両方に流して、出てきた画面をバイト単位で比べるコマンドです（[ヘッドレス実行とゲート](tour-ship.md#ヘッドレス実行とゲート)で扱います）。
このツアーが「コンパイルされる」と書いている機能は、どれもこれを通してあります。
以降の節では、この話は繰り返しません。
