...
補助プロセスを起動するライブラリ（multiprocessing は、何かがロックを取った時点でそうします）が起動するのは、アプリのもう一部ではなくインタプリタになります。
...
- **走り出した task を途中で止めること**。`report` は仕事がどこまで進んだかを伝えますが、止まれと伝える手段はなく、task は最後まで走ります。
