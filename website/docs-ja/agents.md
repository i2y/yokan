...
![端末での一回分。エージェントが app.py を書き、yokan check が直し方を示して拒否し、同じ check が今度は何も言わず、yokan show が画面をテキストで印字し、yokan gate が両方の実行で同じ画面になったと報告する。どの答えもテキストなので、エージェントはそれを読んでまた回る](images/loop-ja.svg#only-dark)

![端末での一回分。エージェントが app.py を書き、yokan check が直し方を示して拒否し、同じ check が今度は何も言わず、yokan show が画面をテキストで印字し、yokan gate が両方の実行で同じ画面になったと報告する。どの答えもテキストなので、エージェントはそれを読んでまた回る](images/loop-ja-light.svg#only-light)
...
アプリが import するモジュールをすべて読み、最初に見つけた拒否を印字します。
...
だから「左を押したまま2フレーム進んだとき、画面に何があるか」も、そのまま印字して確かめられます。
