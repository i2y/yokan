    ...
    F: FnOnce(&mut PlatformInputHandler) -> R,
{
    ...
    if let Some(mut input_handler) = lock.input_handler.take() {
        drop(lock);
        // pixie P1 (carried patch): every NSTextInputClient callback
        // funnels through here, and those callbacks are plain
        // `extern "C"` — a panic crossing them is process death
        // (panic_cannot_unwind). Contain the panic at the funnel and
        // restore the handler either way, so one bad IME interaction
        // degrades to a no-op instead of an abort.
        ...
        match result {
            ...
            Err(_) => {
                log::error!(
                    "gpui_macos: panic in an input handler was contained (pixie P1)"
                );
                None
            }
        }
