...
- `TOUR.md` / `TOUR.ja.md` are the tour, peers; `go run ./tools/tourcheck
  ...
  it.
- `website/` is Gomamochi's own zensical site (`just gomamochi-site`,
  `just gomamochi-site-serve` on :8004), deployed under Yokan's Pages at
  `i2y.github.io/yokan/gomamochi/` and written so it can move to its own
  repository by changing `site_url` and the two switcher links. Four of
  its pages are generated by `just gomamochi-site-gen` — the six tour
  pages cut from `TOUR*.md`, `elements.md` from the table, `demos.md`
  from `demo/`, `refusals.md` from `test/refuse/` — by Go programs under
  `website/tools/`, and the sweep runs `website/tools/sitecheck`, which
  fails when any of them is behind its source, and gates the site's tour
  pages along with the repository's. The gallery under
  `demo/screenshots/` is mirrored into `website/docs*/images/demos/`.
...
- Anything under `gomamochi/` → the touched demo's gate, then
  ...
  fails on a stale table. A change to the tour, the table, a demo or a
  refusal moves a site page too: run `just gomamochi-site-gen`, which
  the sweep only checks. A change to the door or the elements
