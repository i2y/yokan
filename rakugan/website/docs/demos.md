...
#### counter — the reference: an app is a class, its state is its fields, and a handler is an anonymous sub that closes over them
...
#### control — ordinary Perl inside a view: `if`, `unless`, a conditional expression, a loop, and a method that answers part of the screen
...
#### calc — a calculator: one accumulator, one pending operation, and a look kept in a hash and handed to each key
...
#### mixer — state an app keeps, and a field that writes into it; one part of the screen is only there some of the time
...
#### cards — a piece of screen with a name is a method, and one that wraps other elements takes them after its own values
...
#### styled — a look kept in one place: a hash of keywords handed to an element, and `theme` flipping a whole panel
...
#### shared — the keywords every element takes, on elements that have nothing else in common
...
#### stdlib — Perl's own under the gate: `sprintf`, `sort`, `grep`, `map`, `List::Util`, `POSIX`, regular expressions
...
#### files — files through the framework's own library: one implementation answers both runs
...
#### reader — nested JSON, reached by path, written to a file and read back so both runs read the same bytes
...
#### flow — control flow in the handlers: a loop that skips, a loop that stops, a while, and a method that answers a value
