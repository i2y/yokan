...
XS モジュールは、コマンド自身がアプリの perl 向けに `~/.cache/rakugan/door/<バージョン>/` へビルドし、元のファイルが変わればビルドし直します。
...
| 全部を 1 回通す（デモ 41 本、二つのツアー、サイト） | 3 分 13 秒 |
...
すべてのデモを両方の実行で動かし、断りの文面を置いてあるファイル、perl が出力した表、そして二つのツアーの完全な例を、すべて確かめます。
要素の表や XS モジュール、エンジンに手を入れたときに通すものですが、手元の用意が揃っているかを確かめるいちばん速い方法でもあります。
