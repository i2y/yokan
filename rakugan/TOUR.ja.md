...
`rakugan build` は書かれた Perl を [pixie](../docs/PIXIE.md) に翻訳し、Zed エディタを支える **gpui** の上に組んだそのエンジンと結びます。
...
書ける Perl の範囲がこのページの説明する方言で、そこから外れるものは[落雁が断る書き方](#落雁が断る書き方)に挙げてあります。
...
`view` は要素を一つ返し、そのインスタンスを `run` に渡すとウィンドウが開きます。
...
残りはすべて計算されます。
`my` の名前は渡されたものの型になり、式の型はその部品から決まります。
食い違いは、実行して初めて分かるのではなく、行番号とともに示されます。

## ビューの書き方

コンテナは子を引数として取り、キーワードはそのあとに続きます。
要素を返すメソッドは画面の一部分であり、それを呼ぶのがビューを分ける方法です。
...
同じ画面を二度組み立てたら同じ画面になる必要があるからです。
フィールドを書き換えるメソッドはハンドラのもので、断りの文面もそう言います。

## ビューの中の制御構造

ビューの中でも、Perl は Perl です。
...
どれも、表示する値を受け取ります。
変わった値は、書かれたサブルーチンに渡ってきます。
裏で結ばれているものは何もありません。
欄が `$name` を表示するのは `$name` と書いたからで、書き戻すのはそのサブルーチンです。
選択肢のリストはリファレンス（`\@colors`）で渡すか、その場に書きます（`["day", "week"]`）。

## ハンドラ

ハンドラは、要素がそのために用意したキーワードに書く無名サブルーチンです。
受け取る引数は、その出来事が運ぶものだけです。
...
`list_view` と `table` は行数と、`$i` 行目を組み立てるサブルーチンを取ります。
...
`table` は同じかたちに見出しと並べ替えのついたもので、`data_table` は行を自分で組み立てる表です。
行のサブルーチンは画面を描いている最中に呼ばれるので、状態を読むだけで、書き換えることはありません。
使える添字も自分の番号だけです。
リストの中に収まっているとビューが示せない添字は断られ、ハンドラの側で計算することになります。
...
`$prices{$k}` だけの読みは断ります。
その鍵がないかもしれず、ないときに何が起きるかで二つの実行が一致する必要があるからです。
`// 0` が答えを決め、あるかどうかは `exists` が答えます。
`keys` は `sort keys %prices` と書きます。
perl が鍵を返す順序はそのとき保持している順序で、perl を起動するたびに変わります。
...
コンパイルした実行では、二つの名前が共有できるものではなく、値として持ちます。
...
パターン自体は翻訳のときにコンパイルされるので、二つの実行は一つのエンジンで一致を取ります。
ここから二つのことが決まります。
変数から組み立てたパターンは断ります。
配ったアプリには、それをコンパイルするものが入っていないからです。
置換の `/e` も同じ理由で断ります。
`if` の外の `$1` も断ります。
どこかで最後に成功した一致が残したものになり、二つの実行をそれで縛ることはできないからです。
...
名前が Perl のものであるかぎり、仕様は perl です。
...
どれも perl 自身が印字した表（`crates/rakugan-stdlib/tests/expected/`、1000 行あまり）に照らされ、`cargo test -p rakugan-stdlib` が突き合わせます。
答えがマシンの地域設定や時計で変わる関数は、どちらの実行でも UTC を読みます。

## 枠組みの標準ライブラリ

ファイル、データベース、ネットワークなど、オペレーティングシステムの持ち物は、Perl ではなく枠組みから来ます。
そしてそこでは、一つの実装が両方の実行に答えます。
...
素のほうは、失敗したときにそのハンドラを止めます。
`_or` のほうは、代わりに答えるものを受け取ります。
どちらを書くかは、ファイルがないことを誤りと見るか既定値と見るか、という問いです。
...
二つの実行は一つの時計で刻みます。
ウィンドウでは 1 フレーム、スクリプトでは `advance:` の 1 手順です。
...
`rakugan run` はアプリのファイルを見ています。
保存すると、ウィンドウがその編集を取り込みます。
ファイルが読み直され、クラスも読み直され、ウィンドウの持っているインスタンスが新しい `view` で答えます。
コンパイルできないファイルは、ウィンドウをそのままにして、端末にそう出ます。
次にコンパイルできる保存をすれば、それが効きます。
...
先に読むのは perl です。
perl が構文の判定を持っているので、perl が撥ねた書き方が翻訳器に届くことはありません。
...
- 初期値のないフィールド、アプリ自身のクラスのフィールドに付けた `:param`。
  型は初期値から来ます。
...
- 片側が文字列の `+`。
  文字列を数として読むのは `0 + $s` です。
- 文字列への `++`。
  perl はそこで文字を数えますが（`"az"++` は `"ba"`）、コンパイルした実行はそれをしません。
- `//` のないハッシュの読みと、`sort` のない `keys`。
- リストの中に収まっているとビューが示せない添字。
...
- サブルーチンでないハンドラと、呼ばれ方と引数の数が合わないハンドラ。
...
- 方言は部分集合で、[落雁が断る書き方](#落雁が断る書き方)がその外側です。
  どれも、翻訳器がまだコンパイルする実行まで運べない書き方であって、Perl についての評価ではありません。
- リファレンスは、ここに挙げたものだけです。
